...
async function render(keypress: KeyboardEvent) {
...
async function addClasstoLine(_class: string) {
  for (let i = 5 * game.pos[0] + 1; i < game.pos[1]; i++) {
    Promise.resolve(addClasstoBox(i, _class));
  }
}
async function addClasstoBox(i: string | number, _class: string) {
  let box = document.getElementById(`${i}`);
  box?.classList.add(_class);
}

async function addClasstoChar(i: string, _class: string) {
  let char = document.getElementById(`${i}`);
...
async function writeOut(text: string) {
